...
export function FileSettingsForm({ teamId, teamName, commentsEnabled, memberTaskViewEnabled, initialLimit }: { teamId: string; teamName: string; commentsEnabled: boolean; memberTaskViewEnabled: boolean; initialLimit: number }) {
  ...
  return (
    <form action={action} className="rounded-lg border border-border bg-surface p-3">
      ...
      {memberTaskViewEnabled ? <input type="hidden" name="memberTaskViewEnabled" value="on" /> : null}
